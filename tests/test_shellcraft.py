...
def test_game_run(game):
    ...
    for command in commands.splitlines():
        assert not command or command.split()[0] in game.state.commands_enabled, (command.split()[0], type(command.split()[0]), type, game.state.commands_enabled, map(type, game.state.commands_enabled))
